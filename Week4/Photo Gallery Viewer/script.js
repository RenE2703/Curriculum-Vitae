...
function toggleFavorite(fullUrl) {
  const index = favorites.indexOf(fullUrl);
  if (index > -1) {
    favorites.splice(index, 1);
  } else {
    favorites.push(fullUrl);
  }
  saveFavorites();
  updateFavoriteUI();
}
...
function updateFavoriteUI() {
  const currentFullUrl = galleryItems[currentIndex]?.dataset.full || "";
  const isFavorited = favorites.includes(currentFullUrl);
  
  document.querySelectorAll(".gallery__item").forEach((item) => {
    if (favorites.includes(item.dataset.full)) {
...
const downloadImage = () => {
  ...
  const caption = item.querySelector("figcaption")?.textContent || "photo";
...
favoritesBtn.addEventListener("click", () => {
  const favorited = galleryItems.filter((item) => favorites.includes(item.dataset.full));
  if (favorited.length === 0) {
    alert("No favorite photos yet. Add some by clicking the heart button!");
    return;
  }
  
  galleryItems = favorited;
  galleryEl.innerHTML = "";
  galleryItems.forEach((item) => galleryEl.appendChild(item));
  refreshThumbnails();
  updateCounter();
  if (!lightbox.classList.contains("hidden")) {
    currentIndex = 0;
